...
void Peak::logExtent(
  ...
  const Peak& nextPeak)
{
  if (prevPeak.index != index)
  {
    indexLeft = prevPeak.index;
    valueLeft = prevPeak.value;
  }

  if (nextPeak.index != index)
  {
    indexRight = nextPeak.index;
    valueRight = nextPeak.value;
  }
}
