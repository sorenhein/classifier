...
// <http://w...content-available-to-author-only...e.org/licenses/mit-license>
// https://ideone.com/8VVEa
...
Mediator* MediatorNew(int nItems)
{
   ...
   Mediator* m=  static_cast<Mediator *>(malloc(size));
   m->data= (Item*)(m+1);
...
void GetMediatorStats(
  ...
  MediatorStats * ms)
{
  ms->min = m->data[m->heap[1]];
  ms->max = m->data[m->heap[-1]];
  ms->median = MediatorMedian(m);
}
